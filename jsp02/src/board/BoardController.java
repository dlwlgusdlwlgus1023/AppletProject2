...
public class BoardController extends HttpServlet {
	...
	ServletException, IOException {
		...
		if(url.indexOf("list.do") != -1) {
			...
			//파일업로드 처리
			File uploadDir=new File("D:\\work_java\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\jsp02\\upload");
			if(!uploadDir.exists()) {//업로드디렉토리가 존재하지 않으면
				uploadDir.mkdir();//디렉토리를 만듦
			}
			//request를 확장시킨 MultipartRequest 생성
			MultipartRequest multi=new MultipartRequest(request,
					"D:\\work_java\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\jsp02\\upload", Constants.MAX_UPLOAD, "utf-8", 
					new DefaultFileRenamePolicy());
			//request.getParameter대신 multi.getParameter로 바꿔야함.
			//MultipartRequest를 쓰는 순간 request객체가 확장됨
			String writer=multi.getParameter("writer");
			String down=multi.getParameter("down");
			...
			//다운로드할 파일 경로
			String path="D:\\work_java\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\jsp02\\upload\\"+filename;
			...
			//디렉토리가 없으면 생성
			File uploadDir=
			new File("D:\\work_java\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\jsp02\\upload");
			...
			if(fileDel != null && fileDel.equals("on")) {
				String fileName=dao.getFileName(num);
				File f=new File("D:\\work_java\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\jsp02\\upload\\"+fileName);
				f.delete();//파일 삭제
				//첨부파일 정보를 지움
				dto.setFilename("-");
				dto.setFilesize(0);
			...
			MultipartRequest multi=new MultipartRequest(request,
					"D:\\work_java\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\jsp02\\upload", Constants.MAX_UPLOAD, 
